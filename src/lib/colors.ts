// CSS color keywords + a few common aliases. Mirrors src-tauri/src/colors.rs.
export const COLOR_NAMES: Record<string, [number, number, number]> = {
...
export function parseColorQuery(q: string): [number, number, number] | null {
  const s = q.trim().toLowerCase();
  if (!s) return null;
  if (s.startsWith("#") || /^[0-9a-f]{6}$/.test(s) || /^[0-9a-f]{3}$/.test(s)) {
    return parseHex(s);
  }
